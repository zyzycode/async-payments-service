...
def register_handlers(broker: RabbitBroker) -> None:
    ...
    async def process_payment(message: PaymentNewMessage) -> None:
        ...
        Processing:
            ...
            2. Если платеж уже не `pending`, не вызывает gateway повторно и
               сразу переходит к отправке webhook.
        ...
        Ошибки:
            ...
            сообщение не было подтверждено. После успешного сохранения
            финального статуса retry из-за webhook-ошибки повторяет только
            webhook, без повторной эмуляции платежного шлюза.
        """
        try:
